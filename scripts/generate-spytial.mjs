...
export function generate(manifest) {
  ...
  function record(typeName, ctor, fields) {
    ...
    fields.forEach((f, i) => {
      const values = fields.map(g => g === f ? (f.required ? 'new-value' : 'some(new-value)') : `self.${g.pyretName}`);
      lines.push(`    method with-${f.pyretName}(self, new-value :: ${ann(f)}) -> ${typeName}:`,
        `      ${ctor}(${values.join(', ')})`, `    end${i < fields.length - 1 ? ',' : ''}`);
    });
    lines.push('end');
    if (fields.every(f => !f.required)) lines.push(`default-${ctor} = ${ctor}(${fields.map(() => 'none').join(', ')})`);
    lines.push('');
  }
